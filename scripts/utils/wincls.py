...
class yolo_swin:
    ...
    def swinpred(this, img, stride=112, mode='prob', patch_save_path=None):
        """
        ...
        counter = 0
        for y in tqdm.tqdm(range(0, h, stride)):
    ...
    def visualize(this, img, semantic_mask, title="Classifier Swin-Segmentation", save_path=None):
        ...
        line_height = 30
        x_start, y_start = 20, 25
        x_ptr, y_ptr = x_start, y_start
        item_spacing = 150
        rows = (len(this.rdict) * item_spacing // target_w) + 1
        footer_h = rows * line_height + 20
        ...
        for i, name in this.rdict.items():
            ...
            cv2.rectangle(footer, (x_ptr, y_ptr - 15), (x_ptr + 20, y_ptr + 5), c, -1)
...
if __name__ == '__main__':
    mod=yolo_swin("/run/media/lasercat/writebuffer/tmp/wiertenc/best.pt",[120,180]);
    print(mod.rdict);
    dbg="/run/media/lasercat/writebuffer/tmp/wiertenc/debugyolo/"
    im=cv2.imread("/run/media/lasercat/writebuffer/tmp/wiertenc/2026-01-31_13:17:24_0c795f8f2aee317125c50b63dfd38b24884b5723758e31e7d71c7a817fcb8146.jpg");
    # im=cv2.cvtColor(im,cv2.COLOR_BGR2RGB);
    im=im[100:];
    mask=mod.swinpred(im,30,"prob",dbg);
    mod.visualize(im,mask);