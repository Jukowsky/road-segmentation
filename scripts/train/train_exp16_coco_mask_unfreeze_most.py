# exp16_coco_mask_unfreeze_most.py
# COCO masks + freeze=3 (first two blocks only)
...
model.train(
    ...
    batch=32,
    device=2,
    name="exp16_coco_mask_unfreeze_most",

    # ── Freeze only first two blocks (layers 0-2) ─────────────────────
    freeze=3,

    # ── Optimizer — lower LR, most backbone active ────────────────────
    optimizer="SGD",
    lr0=0.0001,
    lrf=0.01,
    momentum=0.937,
    ...
    # ── Augmentation — identical to Exp 12/14 ────────────────────────
    mosaic=1.0,
    mixup=0.1,
    copy_paste=0.1,
    hsv_h=0.015,
